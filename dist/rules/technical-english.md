...
Subagents spawned by the agent tool inherit this file through the `AGENTS.md` import, so do not
restate it in a delegation prompt. They see the version loaded when the session started, so restate
a rule you changed mid-session. When a subagent's prose disobeys these rules, restate the ones that
apply.

    ...
    try:
        with open(sys.argv[3], "w", encoding="utf-8") as f:
            f.write(
                ...
                " above. If the new line is the same item in new words, restate it"
                " with the title on file, word for word, and add no erratum, because"
                " nothing it claimed was wrong. If the new line corrects the one on"
                " file, restate the new line"
